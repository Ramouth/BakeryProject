...
from backend.models import db, Bakery
from backend.schemas import BakerySchema, PastrySchema
...
@cache.cached(timeout=60, key_prefix='get_bakery')
...
def update_bakery(bakery_id):
    ...
    if not CAN_EDIT:
        return jsonify({"message": "Updating bakeries is not allowed"}), 403
